 ...
 */
#ifndef	_NTDDK_
// Unless _NTDDK_ is defined, we are included in a DLL w/o a DDK here,
// so the following fragment is taken from the "devioctl.h" file from the
// MSVC's DDK package.
//
...
#define RFM_PCIOPREGSIZ (48 * 4)	/* Size of PCI bus oper regs 192 */
...
// #define RFM_LCSR1_CONFIG_256M    0;
...
/*
 *------------------------------------------------------------------------
 * The IOCTL_PCI55XX_NNN() macro defines a tool to construct the
...
#define IOCTL_PCI55XX_NNN(offset, method, access) \
    (ULONG) CTL_CODE(FILE_DEVICE_PCI5565, PCI55XX_IOCTL_BASE + (offset), \
		     (method), (access))
...
/*
 *------------------------------------------------------------------------
 * The IOCTL_INTERNAL_PCI55XX_ENABLE_RMNET command is intended for VMIC
 * use only. Customer applications programs must not use this function.
 *------------------------------------------------------------------------
 */

typedef void (*rfmIntCallBack_t)(
	IN PKDPC 		    Dpc,	    /* DPC pointer			     */
	IN PDEVICE_OBJECT 	devObj,	    /* Device object		     */
	IN PIRP			    irp,	    /* I/O Request packet		 */
	IN PVOID		    Context,    /* Deferred context		     */
	IN UCHAR		    eventId,    /* Interrupt event ID code	 */
	IN UCHAR		    senderId    /* Interrupt sender (if any) */
);
